...
describe('ng-cy-mock-data', () => {
  beforeEach(() => {
    cy.fixture("get-bucket")
      .then((response) => {
        cy.intercept('GET', 'http://localhost:3333/api/bucket', response)
        return cy.fixture("add-bucket-item");
      })
      .then((response) => {
        cy.intercept('POST', 'http://localhost:3333/api/bucket', response)
        return cy.fixture("delete-bucket-item");
      })
      .then((response) => {
        cy.intercept('DELETE', 'http://localhost:3333/api/bucket/*', response)
      })
      .visit('/')
  });
